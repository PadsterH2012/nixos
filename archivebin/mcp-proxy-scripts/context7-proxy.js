...
/**
 * Context7 MCP Proxy
 * Connects to centralized Context7 server
 */
...
// Spawn the generic proxy with Context7-specific parameters
const proxy = spawn('node', [proxyScript, serverName, baseUrl], {
  stdio: 'inherit'
});

proxy.on('error', (error) => {
  console.error(`Failed to start Context7 proxy: ${error.message}`);
  process.exit(1);
});
