...
/**
 * mongodb MCP Proxy
 * Connects to centralized mongodb server
 */
...
// Spawn the generic proxy with mongodb-specific parameters
const proxy = spawn('node', [proxyScript, serverName, baseUrl], {
  stdio: 'inherit'
});

proxy.on('error', (error) => {
  console.error(`Failed to start mongodb proxy: ${error.message}`);
  process.exit(1);
});
